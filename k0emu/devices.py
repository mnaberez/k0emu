...
class PortWithEdgeDetectionDevice(PortWithPullupsDevice):
    """GPIO port with pull-ups and external interrupt edge detection.

    Owns the EGP/EGN edge selection registers which control which edges
    trigger interrupts.

    Use set_external_input(pin, state) to change pin levels.  Edge
    detection compares old vs new and fires an interrupt for each pin
    that changed in a direction enabled by EGP/EGN.  Interrupt source
    indices 0-7 correspond to INTP0-INTP7."""
    ...
    def set_external_input(self, pin, state):
        """Set one external input pin and fire an interrupt if the edge matches EGP/EGN."""
        mask = 1 << pin
