...
from k0emu.devices import (MemoryDevice, RegisterFileDevice,
                           ...
                           I2CControllerDevice, PortWithEdgeDetectionDevice,
...
def _make_edge_port_on_bus():
    proc = _FakeProcessor()
    bus = Bus(proc)
    intc = InterruptControllerDevice("intc")
    bus.add_device(intc, (0xFFE0, 0xFFEB))
    bus.set_interrupt_controller(intc)
    port = PortWithEdgeDetectionDevice("p0")
    bus.add_device(port, (0xFF00, 0xFF00), (0xFF20, 0xFF20), (0xFF30, 0xFF30),
                         (0xFF48, 0xFF48), (0xFF49, 0xFF49))
    for bit in range(8):
        intc.connect(port, bit, getattr(intc, 'INTP%d' % bit))
    return port, intc, bus
...
class EdgeDetectionTests(unittest.TestCase):
    ...
    def test_egp_write_read(self):
        port, intc, bus = _make_edge_port_on_bus()
    ...
    def test_rising_edge_fires_when_egp_enabled(self):
        port, intc, bus = _make_edge_port_on_bus()
        port._egp = 0x01
        port.set_external_input(0, False)  # start low
        port.set_external_input(0, True)   # rising edge
        self.assertTrue(self._intp_pending(intc, 0))

    def test_rising_edge_ignored_when_egp_disabled(self):
        port, intc, bus = _make_edge_port_on_bus()
        port.set_external_input(0, False)
        port.set_external_input(0, True)
    ...
    def test_falling_edge_fires_when_egn_enabled(self):
        port, intc, bus = _make_edge_port_on_bus()
        port._egn = 0x01
        port.set_external_input(0, False)  # falling edge (from default high)
        self.assertTrue(self._intp_pending(intc, 0))

    def test_falling_edge_ignored_when_egn_disabled(self):
        port, intc, bus = _make_edge_port_on_bus()
        port.set_external_input(0, False)  # falling but EGN=0
        self.assertFalse(self._intp_pending(intc, 0))
    ...
    def test_no_interrupt_on_same_state_high(self):
        ...
        port._egp = 0xFF
        port._egn = 0xFF
        port.set_external_input(0, True)  # already high
        self.assertFalse(self._intp_pending(intc, 0))

    def test_no_interrupt_on_same_state_low(self):
        port, intc, bus = _make_edge_port_on_bus()
        port.set_external_input(0, False)  # go low
        port._egp = 0xFF
        port._egn = 0xFF
        port.set_external_input(0, False)  # already low
        self.assertFalse(self._intp_pending(intc, 0))
    ...
    def test_both_edges_fire_when_both_enabled(self):
        ...
        port._egp = 0x01
        port._egn = 0x01
        port.set_external_input(0, False)  # falling edge
        self.assertTrue(self._intp_pending(intc, 0))
    ...
    def test_edge_polarity_toggle(self):
        ...
        port, intc, bus = _make_edge_port_on_bus()
        port._egp = 0x01
        port.set_external_input(0, False)  # go low first
        port.set_external_input(0, True)   # rising edge fires
        self.assertTrue(self._intp_pending(intc, 0))
        self._clear_intp(intc, 0)
        # Toggle to falling edge (as firmware does)
        port._egp = 0x00
        port._egn = 0x01
        port.set_external_input(0, False)  # falling edge fires
        self.assertTrue(self._intp_pending(intc, 0))
    ...
    def test_set_external_input_updates_external_inputs(self):
        port, intc, bus = _make_edge_port_on_bus()
        self.assertEqual(port.external_inputs & 0x01, 1)  # default high
        port.set_external_input(0, False)
        self.assertEqual(port.external_inputs & 0x01, 0)
        port.set_external_input(0, True)
        self.assertEqual(port.external_inputs & 0x01, 1)

    def test_set_external_input_preserves_other_pins(self):
        port, intc, bus = _make_edge_port_on_bus()
        port.set_external_input(0, False)
        port.set_external_input(4, False)
        self.assertEqual(port.external_inputs & 0x11, 0x00)
        port.set_external_input(0, True)
        self.assertEqual(port.external_inputs & 0x11, 0x01)
